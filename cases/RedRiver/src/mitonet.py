...
class don_nn(tf.keras.models.Model):
    def __init__(self, l_factor, latent_dim, branch_par_input_shape, branch_ic_input_shape, branch_bc1_input_shape, branch_bc2_input_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer, b_encoder_layers, b_encoder_neurons, b_encoder_actf, b_encoder_init, b_encoder_regularizer, trunk_input_shape, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer, t_encoder_layers, t_encoder_neurons, t_encoder_actf, t_encoder_init, t_encoder_regularizer, dropout=False, dropout_rate=0.1, **kwargs):
        ...
        self.branch_encoder_bc1 = self.Branch_Encoder(branch_bc1_input_shape, b_neurons_layer, b_encoder_layers, b_encoder_neurons, b_encoder_actf, b_encoder_init, b_encoder_regularizer)

        self.branch_encoder_bc2 = self.Branch_Encoder(branch_bc2_input_shape, b_neurons_layer, b_encoder_layers, b_encoder_neurons, b_encoder_actf, b_encoder_init, b_encoder_regularizer)
        ...
        self.branch_bc1 = self.MLP_Branch_BC1(branch_bc1_input_shape, trunk_input_shape, l_factor*latent_dim, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer)

        self.branch_bc2 = self.MLP_Branch_BC2(branch_bc2_input_shape, trunk_input_shape, l_factor*latent_dim, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer)
        
        self.trunk = self.MLP_Trunk(branch_par_input_shape, branch_ic_input_shape, branch_bc1_input_shape, branch_bc2_input_shape, trunk_input_shape, l_factor*latent_dim, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer)
    ...
    def MLP_Branch_BC1(self, branch_bc1_input_shape, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):

        branch_bc1_input_layer = tf.keras.layers.Input(branch_bc1_input_shape,name='branch_bc1_input')
        trunk_input_layer = tf.keras.layers.Input(trunk_input_shape,name='trunk_input')
        x = branch_bc1_input_layer
        for i in range(number_layers):
            x = tf.keras.layers.Dense(neurons_layer,activation=actf,kernel_initializer=init,kernel_regularizer=regularizer,name='branch_hidden'+str(i))(x)
            ones = tf.ones_like(x)
            x1 = tf.keras.layers.Subtract(name='subtract_branch'+str(i))([ones,x])
            x2 = tf.keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,self.branch_encoder_bc1(branch_bc1_input_layer)])
            x3 = tf.keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,self.trunk_encoder(trunk_input_layer)])
            x = tf.keras.layers.Add(name='add_product'+str(i))([x2,x3])
            if self.dropout==True:
                x = tf.keras.layers.Dropout(self.dropout_rate)(x)
        output_layer = tf.keras.layers.Dense(output_shape,kernel_initializer=init,kernel_regularizer=regularizer,name='output_branch')(x)
        model = tf.keras.Model([branch_bc1_input_layer, trunk_input_layer],output_layer)
        return model    

    def MLP_Branch_BC2(self, branch_bc2_input_shape, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):

        branch_bc2_input_layer = tf.keras.layers.Input(branch_bc2_input_shape,name='branch_bc2_input')
        trunk_input_layer = tf.keras.layers.Input(trunk_input_shape,name='trunk_input')
        x = branch_bc2_input_layer
        for i in range(number_layers):
            x = tf.keras.layers.Dense(neurons_layer,activation=actf,kernel_initializer=init,kernel_regularizer=regularizer,name='branch_hidden'+str(i))(x)
            ones = tf.ones_like(x)
            x1 = tf.keras.layers.Subtract(name='subtract_branch'+str(i))([ones,x])
            x2 = tf.keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,self.branch_encoder_bc2(branch_bc2_input_layer)])
            x3 = tf.keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,self.trunk_encoder(trunk_input_layer)])
            x = tf.keras.layers.Add(name='add_product'+str(i))([x2,x3])
            if self.dropout==True:
                x = tf.keras.layers.Dropout(self.dropout_rate)(x)
        output_layer = tf.keras.layers.Dense(output_shape,kernel_initializer=init,kernel_regularizer=regularizer,name='output_branch')(x)
        model = tf.keras.Model([branch_bc2_input_layer, trunk_input_layer],output_layer)
        return model 
        
    def MLP_Trunk(self, branch_par_input_shape, branch_ic_input_shape, branch_bc1_input_shape, branch_bc2_input_shape, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):
        branch_par_input_layer = tf.keras.layers.Input(branch_par_input_shape,name='branch_par_input')
        branch_ic_input_layer = tf.keras.layers.Input(branch_ic_input_shape,name='branch_ic_input')
        branch_bc1_input_layer = tf.keras.layers.Input(branch_bc1_input_shape,name='branch_bc1_input')
        branch_bc2_input_layer = tf.keras.layers.Input(branch_bc2_input_shape,name='branch_bc2_input')
        ...
        x = trunk_input_layer
        for i in range(number_layers):
            x = tf.keras.layers.Dense(neurons_layer,activation=actf,kernel_initializer=init,kernel_regularizer=regularizer,name='trunk_hidden'+str(i))(x)
            ones = tf.ones_like(x)
            x1 = tf.keras.layers.Subtract(name='subtract_branch'+str(i))([ones,x])
            x2 = tf.keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,self.branch_encoder_par(branch_par_input_layer),
                                                                                  self.branch_encoder_ic(branch_ic_input_layer),
                                                                                  self.branch_encoder_bc1(branch_bc1_input_layer),
                                                                                  self.branch_encoder_bc2(branch_bc2_input_layer)])
            x3 = tf.keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,self.trunk_encoder(trunk_input_layer)])  
            x = tf.keras.layers.Add(name='add_product'+str(i))([x2,x3])
            if self.dropout==True: 
                x = tf.keras.layers.Dropout(self.dropout_rate)(x)
        ...
        model = tf.keras.Model([branch_par_input_layer, 
                                branch_ic_input_layer, 
                                branch_bc1_input_layer, 
                                branch_bc2_input_layer, 
                                trunk_input_layer],
                               output_layer)
        return model
        
    def call(self,data):
        par, ic, bc1, bc2, t = data
        br_par = self.branch_par([par,t])
        br_ic = self.branch_ic([ic,t])
        br_bc1 = self.branch_bc1([bc1,t])
        br_bc2 = self.branch_bc2([bc2,t])
        branch = br_par*br_ic*br_bc1*br_bc2
...
class don_model(tf.keras.Model):
    ...
    def train_step(self, dataset):
        ...
        self.b_par_input, self.b_ic_input, self.b_bc1_input, self.b_bc2_input, self.t_input, self.target = dataset
        
        with tf.GradientTape() as tape:
            o_res = self.model([self.b_par_input, self.b_ic_input, self.b_bc1_input, self.b_bc2_input, self.t_input], training=True)
    ...
    def test_step(self, dataset):
        ...
        val = self.model([self.b_par_input, self.b_ic_input, self.b_bc1_input, self.b_bc2_input, self.t_input], training=True)
