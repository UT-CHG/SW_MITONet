...
import settings_optuna_mio as sett
...
mio_tuner_epochs = sett.mio_tuner_epochs
mio_trials = sett.mio_trials
...
def NN(trial):
    ...
    output_shape = trial.suggest_int("output_shape", 
                                        sett.output_shape_lower, 
                                        sett.output_shape_upper,
                                        step = sett.output_shape_step) 
...
study.optimize(objective, n_trials=mio_trials, timeout=None, gc_after_trial=True)

#
original_stdout = sys.stdout

sys.stdout = open("mio_optuna.txt", "w")
#
...
with open('mio_trials.txt', 'a') as f:
