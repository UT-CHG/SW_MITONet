...
ae_model_dir = "saved_models_mito"
mito_model_dir = "saved_models_mito"
scalers_dir = "scalers_mito"

#Load custom modules
import mitonet as mito
import autoencoder as ae
import data_loader as dl 
import settings_optuna_mito as sett
...
mito_tuner_epochs = sett.mito_tuner_epochs
mito_trials = sett.mito_trials
...
window_size=5
b_par_train, b_ic_train, b_bc_train, t_train, target_train = pu.latent_multiple_param_windows(param_train,train_ls,train_snaps[0],train_snaps[1],data_dir,window_s=window_size)
b_par_val, b_ic_val, b_bc_val, t_val, target_val = pu.latent_multiple_param_windows(param_val,val_ls,val_snaps[0],val_snaps[1],data_dir,window_s=window_size)
...
# Add trial for scaling?
    ...
    dataset = tf.data.Dataset.from_tensor_slices((
        tf.convert_to_tensor(b_par_train, dtype=tf.float32),
        tf.convert_to_tensor(b_ic_train, dtype=tf.float32),
        tf.convert_to_tensor(b_bc_train, dtype=tf.float32),
        tf.convert_to_tensor(t_train, dtype=tf.float32),
        tf.convert_to_tensor(target_train, dtype=tf.float32)))
    dataset = dataset.shuffle(buffer_size=int(t_train.shape[0])).batch(batch_size)
    
    val_dataset = tf.data.Dataset.from_tensor_slices((
        tf.convert_to_tensor(b_par_val, dtype=tf.float32),
        tf.convert_to_tensor(b_ic_val, dtype=tf.float32),
        tf.convert_to_tensor(b_bc_val, dtype=tf.float32),
        tf.convert_to_tensor(t_val, dtype=tf.float32),
        tf.convert_to_tensor(target_val, dtype=tf.float32)))
...
# Deifne search parameters
study = optuna.create_study(direction="minimize", pruner=optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=500))
study.optimize(objective, n_trials=mito_trials, timeout=None, gc_after_trial=True)

#
original_stdout = sys.stdout

sys.stdout = open("mito_optuna.txt", "w")
#
...
#export DataFrame to text file (keep header row and index column)
with open('mito_trials.txt', 'a') as f:
    df_string = study.trials_dataframe().sort_values("value").to_string()
    f.write(df_string)
