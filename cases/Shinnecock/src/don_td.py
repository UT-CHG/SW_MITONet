...
class don_nn(tf.keras.models.Model):
    def __init__(self, window_size, branch_input_shape, b_number_layers, b_neurons_layer, branch_output_shape, b_actf, b_init, b_regularizer, trunk_input_shape, t_number_layers, t_neurons_layer, trunk_output_shape, t_actf, t_init, t_regularizer, dropout=False, dropout_rate=0.1, **kwargs):
        ...
        self.branch = self.MLP_Branch(window_size, branch_input_shape, branch_output_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer)

        
        self.trunk = self.MLP_Trunk(window_size, trunk_input_shape, trunk_output_shape, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer)
        
        self.b0 = tf.Variable(tf.zeros(1,dtype=tf.float32), shape=tf.TensorShape(1), name='b0', dtype=tf.float32)   
            
    def MLP_Branch(self, window_size, branch_input_shape, branch_output_shape, number_layers, neurons_layer, actf, init, regularizer):
        branch_input_layer = tf.keras.layers.Input(shape=(window_size,branch_input_shape),name='branch_input')
        ...
        output_layer = tf.keras.layers.Dense(branch_output_shape,
                                             kernel_initializer=init,
                                             kernel_regularizer=regularizer,
                                             name='output_branch')(x)
        model = tf.keras.Model(branch_input_layer, output_layer)
        
        return model

    def MLP_Trunk(self, window_size, trunk_input_shape, trunk_output_shape, number_layers, neurons_layer, actf, init, regularizer):
        trunk_input_layer = tf.keras.layers.Input(shape=(None,trunk_input_shape),name='trunk_input')
        x = trunk_input_layer
        for i in range(number_layers):
            x = tf.keras.layers.Dense(neurons_layer,activation=actf,kernel_initializer=init,kernel_regularizer=regularizer,name='branch_hidden'+str(i))(x)
            if self.dropout==True: 
    ...
    def call(self,data):
        b, t = data
        onet = tf.einsum('ikp,kjp->ikj', self.branch(b), self.trunk(t)) + self.b0
