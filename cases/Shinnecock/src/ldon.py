...
class ldon_nn(tf.keras.models.Model):
    def __init__(self, l_factor, latent_dim, branch_input_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer, trunk_input_shape, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer, dropout=False, dropout_rate=0.1, **kwargs):
        ...
        self.branch = self.MLP_Branch(branch_input_shape, l_factor*latent_dim, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer)
        
        self.trunk = self.MLP_Trunk(trunk_input_shape, l_factor*latent_dim, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer)
        
        self.b0 = tf.Variable(tf.zeros(latent_dim,dtype=tf.float32), shape=tf.TensorShape(latent_dim), name='b0', dtype=tf.float32)   
            
    def MLP_Branch(self, branch_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):
    ...
    def MLP_Trunk(self, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):
    ...
    def call(self,data):
        b, t = data
        br = tf.reshape(self.branch(b), [-1, self.latent_dim, self.l_factor])# automate the m and p shapes
        tr = tf.reshape(self.trunk(t), [-1, self.latent_dim, self.l_factor])
        onet_full = tf.einsum('ijk,pjk->ipj', br, tr) + self.b0
        onet = tf.reshape(onet_full, [tf.shape(br)[0], -1]) 
        return onet
    
class ldon_model(tf.keras.Model):
    def __init__(self, model):
        super(ldon_model, self).__init__()
        self.model = model

    def compile(self, optimizer, loss_fn):
        super(ldon_model, self).compile()
