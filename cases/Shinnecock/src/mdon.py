...
class mdon_nn(tf.keras.models.Model):
    def __init__(self, branch_input_shape, branch_output_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer, b_encoder_layers, b_encoder_neurons, b_encoder_actf, b_encoder_init, b_encoder_regularizer, trunk_input_shape,  trunk_output_shape, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer, t_encoder_layers, t_encoder_neurons, t_encoder_actf, t_encoder_init, t_encoder_regularizer, dropout=False, dropout_rate=0.1, **kwargs):
        ...
        self.branch = self.MLP_Branch(branch_input_shape, trunk_input_shape, branch_output_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer)
    ...
    def MLP_Branch(self, branch_input_shape, trunk_input_shape, branch_output_shape, number_layers, neurons_layer, actf, init, regularizer):
        ...
        trunk_enc = self.trunk_encoder(trunk_input_layer)
        branch_enc = self.branch_encoder(branch_input_layer)
        
        t_vec = tf.keras.layers.Lambda(lambda z: tf.reduce_mean(z, axis=0, keepdims=True),
                                       name='trunk_reduce_to_vector')(trunk_enc)
        ...
        for i in range(number_layers):
            ...
            x2 = tf.keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,branch_enc])
            x3 = tf.keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,t_vec])  
    ...
    def MLP_Trunk(self, branch_input_shape, trunk_input_shape, trunk_output_shape, number_layers, neurons_layer, actf, init, regularizer):
        branch_input_layer = tf.keras.layers.Input(branch_input_shape,name='branch_input')
        trunk_input_layer = tf.keras.layers.Input(trunk_input_shape,name='trunk_input')
        ...
        b_vec = tf.keras.layers.Lambda(lambda z: tf.reduce_mean(z, axis=0, keepdims=True),
                                       name='branch_reduce_to_vector')(branch_enc)
        ...
        for i in range(number_layers):
            ...
            ones = tf.ones_like(x)
            x1 = tf.keras.layers.Subtract(name='subtract_branch'+str(i))([ones,x])
            x2 = tf.keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,b_vec])
            x3 = tf.keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,trunk_enc])  
            x = tf.keras.layers.Add(name='add_product'+str(i))([x2,x3])
            if self.dropout==True: 
                x = tf.keras.layers.Dropout(self.dropout_rate)(x)
    ...
    def call(self,data):
        onet = tf.einsum('ip,jp->ij', self.branch(data), self.trunk(data)) + self.b0
        return onet
    
class mdon_model(tf.keras.Model):
    def __init__(self, model):
        super(mdon_model, self).__init__()
        self.model = model

    def compile(self, optimizer, loss_fn):
        super(mdon_model, self).compile()
